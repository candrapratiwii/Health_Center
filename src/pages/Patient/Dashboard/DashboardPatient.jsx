  ...
  useEffect(() => {
    ...
    getDataPrivate(`/api/v1/queues/user/${userProfile.id_user}`)
      .then((resp) => {
        ...
        setStats((s) => ({
          ...s,
          activeReservations: data.filter(
            (a) => a.status === "confirmed" || a.status === "pending"
          ).length,
        }));
        // Hitung kunjungan bulan ini: status confirmed dan tanggal di bulan ini
        const now = new Date();
        const thisMonth = now.getMonth();
        const thisYear = now.getFullYear();
        const monthlyVisits = data.filter((a) => {
          if (a.status !== "confirmed" || !a.tanggal_reservasi) return false;
          const tgl = new Date(a.tanggal_reservasi);
          return tgl.getMonth() === thisMonth && tgl.getFullYear() === thisYear;
        }).length;
        setStats((s) => ({ ...s, monthlyVisits }));
        setLoading(false);
      })
      .catch(() => {
        setAppointments([]);
    ...
    getDataPrivate("/api/v1/health_centers").then((resp) => {
      setStats((s) => ({
        ...s,
        healthCenters: Array.isArray(resp)
          ? resp.length
          : resp?.data?.length || 0,
      }));
    });
    getDataPrivate("/api/v1/services").then((resp) => {
      setStats((s) => ({
        ...s,
        availableServices: Array.isArray(resp)
          ? resp.length
          : resp?.data?.length || 0,
      }));
    });
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        ...
        <p>
          Kelola kesehatan Anda dengan mudah melalui sistem reservasi Puskesmas
          Buleleng
        </p>
      </div>
        ...
        >
          ...
          >
            ...
            <ul style={{ margin: "1rem 0 0 1.2rem", color: "#0369a1" }}>
              ...
              <li>
                <b>
                  Diwajibkan membawa kelengkapan persyaratan Pemeriksaan /
                  Pengobatan sebagai berikut:
                </b>
                <ul style={{ marginTop: "0.5rem" }}>
                  <li>
                    Fotokopi Kartu Keluarga (KK) <i>(ketentuan terbaru)</i>
                  </li>
                  <li>
                    Alamat email yang berlaku{" "}
                    <i>(ketentuan terbaru untuk pasien non lansia)</i>
                  </li>
                  <li>Kartu BPJS Kesehatan dan KTP</li>
                  <li>
                    Kartu berobat (bagi pasien lama, umum, dan BPJS Kesehatan)
                  </li>
                  <li>
                    Dokumen rujukan dari rumah sakit dan Kartu Pengambilan Obat
                    (KPO), khusus pasien yang melakukan perpanjangan rujukan
                  </li>
                </ul>
              </li>
            </ul>
          ...
          >
            <b style={{ fontSize: "1.1rem", marginBottom: 12 }}>
              Jadwal Layanan
            </b>
                ...
                .map((app) => (
                  <div className="appointment-card" key={app.id_antrian}>
                    <div className="appointment-info">
                      ...
                      <p className="puskesmas-name">
                        {app.nama_puskesmas || "-"}
                      </p>
                      <p className="appointment-details">
                        {app.tanggal_reservasi
                          ? new Date(app.tanggal_reservasi).toLocaleDateString(
                              "id-ID",
                              {
                                weekday: "long",
                                year: "numeric",
                    ...
                    >
                      <span
                        className={
                          statusMap[app.status]?.className || "status-badge"
                        }
                      ...
                      <div className="queue-number" style={{ marginTop: 8 }}>
                        ...
                        <span className="queue-highlight">
                          {app.nomor_antrian
                            ? app.nomor_antrian.toString().padStart(3, "0")
                            : "-"}
                        </span>
                      </div>
                    </div>
